...
class Graph {
 public:
  ...
  vector<float> avg_com;
vector<float> std_com;
vector<unsigned int> degree_com;
  ...
  float avg_degree_comm(int comm);
  float degree_comm(int node, int comm);

  double variane(int comm);
  double standard_deviation(int comm);

  double z_score(int node, int comm);

double participation(int node);
